          ...
          <div className="animate-fade-in space-y-6" style={{ animationDelay: "300ms" }}>
            ...
            <div className="card p-5">
              ...
              <div className="mt-1 flex justify-between text-[10px] text-surface-400">
                <span>{t("analytics.fraud")} {feedback.total_labeled > 0 ? ((feedback.fraud_count / feedback.total_labeled) * 100).toFixed(1) : 0}%</span>
                <span>{t("analytics.legitimate")} {feedback.total_labeled > 0 ? ((feedback.legitimate_count / feedback.total_labeled) * 100).toFixed(1) : 0}%</span>
              </div>
            </div>
            ...
            <div className="card p-5">
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                {[
                  { label: t("analytics.precision"), value: feedback.precision, color: "text-brand-600 dark:text-brand-400" },
                  { label: t("analytics.recall"), value: feedback.recall, color: "text-emerald-600 dark:text-emerald-400" },
                  { label: t("analytics.f1Score"), value: feedback.f1_score, color: "text-amber-600 dark:text-amber-400" },
                  { label: t("analytics.accuracy"), value: feedback.accuracy, color: "text-violet-600 dark:text-violet-400" },
                ].map((m) => (
            ...
            <div className="card p-5">
              ...
              <div className="mx-auto max-w-xs">
                <div className="mb-2 flex justify-end gap-1 pr-1">
                  <span className="w-[calc(50%-4px)] text-center text-[10px] font-semibold uppercase tracking-wider text-surface-400">{t("analytics.predictedFraud")}</span>
                  <span className="w-[calc(50%-4px)] text-center text-[10px] font-semibold uppercase tracking-wider text-surface-400">{t("analytics.predictedLegit")}</span>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-lg bg-emerald-50 p-4 text-center dark:bg-emerald-950/30">
                    <p className="text-[10px] font-semibold uppercase text-emerald-600 dark:text-emerald-400">{t("analytics.truePositive")}</p>
                  ...
                  <div className="rounded-lg bg-red-50 p-4 text-center dark:bg-red-950/30">
                    <p className="text-[10px] font-semibold uppercase text-red-600 dark:text-red-400">{t("analytics.falseNegative")}</p>
                    <p className="font-mono text-xl font-bold text-red-700 dark:text-red-300">{feedback.false_negatives}</p>
                  </div>
                  <div className="rounded-lg bg-red-50 p-4 text-center dark:bg-red-950/30">
                    <p className="text-[10px] font-semibold uppercase text-red-600 dark:text-red-400">{t("analytics.falsePositive")}</p>
                    <p className="font-mono text-xl font-bold text-red-700 dark:text-red-300">{feedback.false_positives}</p>
                  </div>
                  <div className="rounded-lg bg-emerald-50 p-4 text-center dark:bg-emerald-950/30">
                    <p className="text-[10px] font-semibold uppercase text-emerald-600 dark:text-emerald-400">{t("analytics.trueNegative")}</p>
                ...
                <div className="mt-2 flex gap-1 pl-1">
                  <span className="w-[calc(50%-4px)] text-center text-[10px] font-semibold uppercase tracking-wider text-surface-400">{t("analytics.actualFraud")}</span>
                  <span className="w-[calc(50%-4px)] text-center text-[10px] font-semibold uppercase tracking-wider text-surface-400">{t("analytics.actualLegit")}</span>
                </div>
              </div>
            </div>
          </div>
